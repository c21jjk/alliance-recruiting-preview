...
function RetentionSubTab({ id, active, setActive, label, count, badge, principalOnly }) {
  ...
  return (
    <button
      ...
      className={`px-4 md:px-5 py-3 text-[12px] font-bold tracking-wide transition flex items-center gap-2 border-b-2 whitespace-nowrap ${
        isActive ? 'border-[#C9A84C] text-[#252526]' : 'border-transparent text-gray-500 hover:text-[#252526]'
      }`}
    >
      {label}
      {count !== null && count !== undefined && (
        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
          isActive ? 'bg-[#C9A84C] text-[#252526]' : 'bg-gray-200 text-gray-600'
        }`}>
          {count}
        </span>
      )}
